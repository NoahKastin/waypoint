...
- **Released as v0.2.1** — a *point* release, not a minor: the `hub`→`map` CLI rename is a breaking change but small-scale, and the maintainer versions by proportionality (a small breaking change can ride a patch) rather than strict semver — consistent with how 0.2.0 absorbed its post-bump polish without re-bumping.
